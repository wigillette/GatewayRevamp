...
const initial = {coreAssignments: CORES.reduce((assignments,coreId) => ({...assignments,[coreId]:null}), {}), planMappings: [], totalCredits: 0, message: undefined};
const planReducer = (state = initial, action) => {
    switch(action.type) {
        ...
        case FETCH_ASSIGNMENTS:
            return {...state, coreAssignments: action.payload.coreAssignments, planMappings: action.payload.planMappings, totalCredits: action.payload.totalCredits}
        default:
            return state;
    }
}
